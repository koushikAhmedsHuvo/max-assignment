import { Surah, SurahData, Ayah } from "./types";
...
export async function getSurahWithTranslation(id: number): Promise<SurahData> {
...
export function getAyahAudioUrl(verseNumber: number): string {
  ...
  // Use /audio/proxy endpoint which handles CORS and streams the audio
  const url = `${API_BASE}/audio/proxy/surah/${reference.surahId}/ayah/${reference.ayahNumber}?reciter=mishary`;
  return url;
}
...
function getSurahAyahFromGlobal(globalAyahNumber: number): {
  ...
  if (
    globalAyahNumber < 1 ||
    globalAyahNumber > CUMULATIVE_AYAH_COUNTS[CUMULATIVE_AYAH_COUNTS.length - 1]
  ) {
